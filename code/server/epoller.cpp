...
bool
Epoller::AddFd (int fd_, u_int32_t events)
{
    if (fd_ < 0)
        return false;
    epoll_event ev = { 0 };
    ev.data.fd     = fd_;
    ev.events      = events;
    return 0 == epoll_ctl (epollFd_, EPOLL_CTL_ADD, fd_, &ev);
}

bool
Epoller::ModFd (int fd, u_int32_t events)
{
    if (fd < 0)
        return false;
    epoll_event ev = { 0 };
...
Epoller::DelFd (int fd)
{
    if (fd < 0)
        return false;
    // dont know why add ev, maybe nullptr
    epoll_event ev = { 0 };
