...
// registe event in kernel, Mode: ET, OPTIONAL: EPOLLONESHOT
void
...
// remove fd from time table in kernel
void
...
// RESET event's property to EPOLLONESHOT
void
...
const char *error_400_form
    = "Your request has bas syntax or is inherently impossible to staisfy.\n";
const char *error_403_title = "Forbidden";
const char *error_403_form
    = "You do not have permission to get file from this server.\n";
const char *error_404_title = "Not Found";
const char *error_404_form
    = "The reqested fie was  not found on this server.\n";
const char *error_500_title = "Internal Error";
const char *error_500_form
    = "There was an unusual problem servingthe request file.\n";
...
void
http_conn::initmysql_result (sql_conn_pool *connPool)
{
...
void
http_conn::init (int sockfd, const sockaddr_in &addr, char *root, int TRIGMode,
                 ...
                 std::string sqldbname)
{
    ...
    // when broswer occur RECONECTION, kind of ERROR of root dir/http reponse,
    // or the file is empty that requsetting
    doc_root = root;
    m_TRIGMode = TRIGMode;
...
http_conn::do_request ()
{
    ...
    if (1 == cgi && (*(p + 1) == '2' || *(p + 1) == '3'))
    {
        ...
        // if could find the 'user' and 'passwd' typed on broswer on db
        // TABLE, return 1, else 0
        else if (*(p + 1) == '2')
        {
            if (users.find (name) != users.end ()
                && users[name] == password)
...
http_conn::add_response (const char *format, ...)
{
    ...
    int len
        = vsnprintf (m_write_buf + m_write_idx,
...
http_conn::write ()
{
    ...
    while (1)
    {
        ...
        if (bytes_have_send >= m_iv[0].iov_len)
        {
            m_iv[0].iov_len = 0;
            m_iv[1].iov_base
                = m_file_address + (bytes_to_send - m_write_idx);
