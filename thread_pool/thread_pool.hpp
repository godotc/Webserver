...
{
private:
    int m_thread_numbers;    // number of threads in pool
...
private:
    // the function that work thread run, It constantly fetch work from work queue then deal
    static void* worker(void* arg);
    void run();

public:
    // thread_number is the num of thread in pool
    threadpool(int actor_model , sql_conn_pool* connPool , int thread_numbers = 8 , int max_request = 10000);
    ~threadpool();
...
template<typename T>
threadpool<T>::threadpool(int actor_model , sql_conn_pool* connPool , int thread_number , int max_request)
    :m_actor_model(actor_model) , m_thread_numbers(thread_number) , m_max_requests(max_request) ,
